...
namespace network {
    ...
    public:
        ...
        tcp (synapse::network::socket_handle t_fd,
...
synapse::network::tcp::tcp (synapse::network::socket_handle t_fd  ,
                            synapse::network::ip::v4        t_addr)
    : socket_base(t_fd, t_addr) { }

bool synapse::network::tcp::connect   ()
{
    int     conn_res = ::connect(socket_descriptor, (const sockaddr*)&socket_address.address_context(), sizeof(sockaddr_in));
...
size_t synapse::network::tcp::send(uint8_t* s_ctx, size_t s_size)
{
    ...
    ssize_t snd_res = ::send(socket_descriptor, s_ctx, s_size, 0);
    if     (snd_res < 0)
    {
...
size_t synapse::network::tcp::recv(uint8_t* r_ctx, size_t r_size)
{
    ...
    ssize_t rcv_res = ::recv(socket_descriptor, r_ctx, r_size, 0);
    if     (rcv_res < 0)
    {
        io_state_flag = synapse::io::io_state::io_error;
        return 0;
